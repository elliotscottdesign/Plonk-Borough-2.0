  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18, maxWidth: 820 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div className="serif" style={{ fontSize: 24, color: 'var(--cream)' }}>🪪 Members &amp; Loyalty</div>
      ...
      <div style={{ fontSize: 14, color: 'var(--cream-dim)', lineHeight: 1.65 }}>
        ...
        {[
          ...
          <div key={i} style={{ background: 'var(--ink-2)', border: '1px solid rgba(201,168,76,0.15)', borderRadius: 10, padding: 16 }}>
            ...
            <ul style={{ margin: 0, paddingLeft: 18, display: 'flex', flexDirection: 'column', gap: 6 }}>
              {s.items.map((it, j) => <li key={j} style={{ fontSize: 13, color: 'var(--cream)', lineHeight: 1.5 }}>{it}</li>)}
            </ul>
          </div>
        ))}
      </div>
      ...
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: 16 }}>
        ...
        <ul style={{ margin: 0, paddingLeft: 18, display: 'flex', flexDirection: 'column', gap: 6 }}>
          <li style={{ fontSize: 13, color: 'var(--cream)', lineHeight: 1.5 }}>The data store — same call as the newsletter list (Google Sheet vs Supabase). Supabase is the better home for members + loyalty logic.</li>
          <li style={{ fontSize: 13, color: 'var(--cream)', lineHeight: 1.5 }}>How members earn (per visit, per £ spent, or both) and the rewards you want to offer.</li>
          <li style={{ fontSize: 13, color: 'var(--cream)', lineHeight: 1.5 }}>How they check in / are identified at the bar (QR code, phone number, member code).</li>
        </ul>
      </div>
