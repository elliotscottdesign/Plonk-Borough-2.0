...
const COURSE_IMGS = [
  { src:'/Borough_Course_1.jpg', caption:'Hole 5 ÃÂ¢ÃÂÃÂ Belfast cityscape mural' },
  { src:'/Borough_Course_2.jpg', caption:'The bar arch ÃÂ¢ÃÂÃÂ playing through' },
  { src:'/Borough_Course_3.jpg', caption:'London landmarks course' },
  { src:'/Borough_Course_4.jpg', caption:'London Eye hole ÃÂ¢ÃÂÃÂ arch 4' },
  { src:'/Borough_Course_5.jpg', caption:'Red telephone box feature hole' },
  { src:'/Borough_Course_6.jpg', caption:"It's a London Thing ÃÂ¢ÃÂÃÂ arch 3" },
]

const DRINKS_IMGS = [
  { src:'/drinks_1.jpg', caption:'Camden Town Brewery ÃÂ¢ÃÂÃÂ bar draught' },
  { src:'/drinks_2.jpg', caption:'Cloudwater craft beer selection' },
  { src:'/drinks_3.jpg', caption:"Mondo Brewing ÃÂ¢ÃÂÃÂ Dennis Hopp’r IPA on tap" },
  { src:'/drinks_4.jpg', caption:'Tropical cocktail ÃÂ¢ÃÂÃÂ citrus spritz' },
...
function TabCatchment() {
  const stats = [
    ...
    { value:'ÃÂ£57K', label:'SE1 MEDIAN INCOME', sub:'household income', source:'ONS 2024', color:'#2DD4BF' },
    { value:'48%', label:'AGE 25Ã¢ÂÂÃÂÃÂ44', sub:'of SE1 residents', source:'Census 2021', color:'#4FC3F7' },
  ...
  const strengths = [
    { icon:'ÃÂ°ÃÂÃÂÃÂ¢', title:'City Worker Proximity', text:'The Square Mile is within 10-minute walking distance. Corporate lunch, after-work and team-building bookings are a core revenue opportunity.' },
    { icon:'ÃÂ°ÃÂÃÂÃÂ', title:'Young Professional Base', text:'48% of SE1 residents are aged 25Ã¢ÂÂÃÂÃÂ44 Ã¢ÂÂÃÂÃÂ the primary spending demographic for experience-led hospitality.' },
    { icon:'Ã¢ÂÂÃÂÃÂÃÂ¯ÃÂ¸ÃÂ', title:'International Tourism', text:'Borough Market and The Shard draw millions of international visitors annually. No Dice Borough is a natural tourist magnet Ã¢ÂÂÃÂÃÂ an experience venue with no paid acquisition cost.' },
    { icon:'ÃÂ°ÃÂÃÂÃÂ', title:'Evening Economy', text:'London Bridge is a major night economy hub. Friday and Saturday footfall is exceptional with no enforced curfew on experience venues.' },
  ]
  return (
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      <Card>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
          ...
          <div>
            {[['MonÃÂ¢ÃÂÃÂFri','07:00 ÃÂ¢ÃÂÃÂ 23:30'],['Saturday','07:00 ÃÂ¢ÃÂÃÂ 23:30'],['Sunday','07:00 ÃÂ¢ÃÂÃÂ 23:30'],['DPS','Klaudia Ciepluch'],['OS Map Ref','532733180272']].map(([l,v]) => <Row key={l} label={l} value={v} />)}
          </div>
        </div>
      </Card>
...
function TabFloorPlan() {
  const specs = [
    { label:'UPSTAIRS', value:'400mÃÂ²', color:'#4FC3F7', note:null },
    { label:'DOWNSTAIRS', value:'300mÃÂ²', color:'#C9A84C', note:'SPACE CURRENTLY USED FOR STORES, CELLAR & KITCHEN' },
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, alignItems:'start' }}>
      <div style={{ background:'var(--ink-2)', border:'1px solid rgba(201,168,76,0.12)', borderRadius:10, padding:16 }}>
        <div style={{ fontSize:11, color:'var(--gold-dim)', letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:12 }}>Floor Plan Ã¢ÂÂÃÂÃÂ Arches B C D And E, Montague Close SE1</div>
...
function TabLicence() {
  const a1 = [
    ...
    {r:'485',t:'No irresponsible promotions. The responsible person must ensure that staff do not carry out, arrange or participate in any irresponsible promotions -ÃÂÃÂ including activities encouraging individuals to drink within time limits, provision of unlimited quantities of alcohol at a fixed/discounted fee, or dispensing alcohol directly into another person-ÃÂÃÂs mouth.'},
    ...
    {r:'489',t:'Mandatory measures. Beer/cider: ÃÂ½ pint; Gin/rum/vodka/whisky: 25 ml or 35 ml; Still wine: 125 ml. Measures must be displayed and offered where no quantity is specified by the customer.'},
  ...
  const a2 = [
    ...
    {r:'316',t:'No deliveries or waste collections outside the hours of 8am-ÃÂÃÂ8pm Monday to Saturday or 10am-ÃÂÃÂ4pm on Sunday.'},
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
        <div style={cs}><STitle>Part 1 -ÃÂÃÂ Premises Details</STitle>
          <Row label="Premises Licence" value="Licensing Act 2003 ÃÂ· Issued by Southwark Council" />
        ...
        <div style={cs}><STitle>Part 2 -ÃÂÃÂ Licence Holder</STitle>
      ...
      <div style={cs}><STitle>Licensable Activities</STitle>
        <Row label="Films -ÃÂÃÂ Indoors" value="Permitted" gold />
      ...
      <div style={cs}><STitle>Opening Hours -ÃÂÃÂ For non-standard timings see Annex 2</STitle>
        <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4,marginBottom:16}}>
          {days.map(d=><div key={d} style={{textAlign:'center'}}><div style={{fontSize:11,color:'var(--cream-dim)',marginBottom:4}}>{d}</div><div style={{fontSize:11,color:'var(--cream)',background:'var(--ink-3)',borderRadius:4,padding:'4px 2px'}}>07:00</div><div style={{fontSize:10,color:'var(--gold-dim)',margin:'2px 0'}}>-ÃÂÃÂ</div><div style={{fontSize:11,color:'var(--cream)',background:'var(--ink-3)',borderRadius:4,padding:'4px 2px'}}>23:30</div></div>)}
        </div>
        <STitle>Licensed Activity Hours</STitle>
        {[['Films -ÃÂÃÂ Indoors','11:00-ÃÂÃÂ23:00'],['Indoor Sporting Event','11:00-ÃÂÃÂ23:00'],['Alcohol -ÃÂÃÂ On Premises','11:00-ÃÂÃÂ23:00']].map(([act,hrs])=>(
      ...
      <div style={cs}><STitle>Annex 1 -ÃÂÃÂ Mandatory Conditions</STitle>
      ...
      <div style={cs}><STitle>Annex 2 -ÃÂÃÂ Operating Schedule Conditions</STitle>
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:0, fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ marginBottom:40 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
          <div style={{ background:'#0D1117', border:'1px solid #21262D', borderRadius:10, padding:24, borderTop:'3px solid #C9A84C' }}>
            ...
            {['200mÃÂ² of basement space directly below the bar and arcade arch','Available for development now Ã¢ÂÂÃÂÃÂ no structural barriers','Proposed uses: karaoke rooms ÃÂ· listening bar ÃÂ· private hire games spaces','High-margin private hire and events revenue potential','Extends the venue offer without expanding the footprint above ground'].map(item => (
              <div key={item} style={{ display:'flex', gap:10, marginBottom:10, fontSize:14, color:'#D1D5DB' }}>
                <span style={{ color:'#C9A84C', flexShrink:0 }}>Ã¢ÂÂÃÂÃÂ</span><span>{item}</span>
              </div>
            ))}
          </div>
          <div style={{ background:'#0D1117', border:'1px solid #21262D', borderRadius:10, padding:24, borderTop:'3px solid #4FC3F7' }}>
            ...
            {['300mÃÂ² of developable basement beneath the full Borough venue','Rights to sublet and carry out works are held within the lease','Structural costs covered by the landlord and building insurance policy','Fit-out to be carried out by No Dice Borough Ltd','A further 100mÃÂ² tunnel opens under the final golf arch once TfL installs a new staircase Ã¢ÂÂÃÂÃÂ at that point the entire basement connects for public use','Full basement development unlocks a significant additional revenue floor'].map(item => (
              <div key={item} style={{ display:'flex', gap:10, marginBottom:10, fontSize:14, color:'#D1D5DB' }}>
                <span style={{ color:'#4FC3F7', flexShrink:0 }}>Ã¢ÂÂÃÂÃÂ</span><span>{item}</span>
              </div>
            ))}
          </div>
        </div>
        <div style={{ background:'rgba(45,212,191,0.06)', border:'1px solid rgba(45,212,191,0.2)', borderRadius:8, padding:'16px 20px', marginBottom:12 }}>
          <p style={{ fontSize:13, color:'#9CA3AF', fontStyle:'italic', marginBottom:12 }}>Ã¢ÂÂÃÂÃÂ The basement represents significant embedded upside Ã¢ÂÂÃÂÃÂ it is not speculative. The rights exist, the space exists, and the structural costs are not borne by No Dice Borough Ltd.</p>
          <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
            {['Lease-Backed Rights','Structural Costs: Landlord','400mÃÂ² Total Potential'].map((tag,i) => (
      ...
      <div style={{ marginBottom:40 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
          <div style={{ background:'#0D1117', border:'1px solid #21262D', borderRadius:10, padding:24, borderTop:'3px solid #F87171' }}>
            ...
            <h3 style={{ fontSize:18, fontWeight:700, color:'#fff', marginBottom:16 }}>Boro Bistro Ã¢ÂÂÃÂÃÂ Final Warning</h3>
            {['The yard at the front of the venue is private land owned by Southwark Cathedral','Boro Bistro currently hold usage rights to this space','They have misused the space and are on their final warning with the landlord','One further breach gives No Dice Borough the right to reclaim the yard'].map(item => (
              <div key={item} style={{ display:'flex', gap:10, marginBottom:10, fontSize:14, color:'#D1D5DB' }}>
                <span style={{ color:'#F87171', flexShrink:0 }}>Ã¢ÂÂÃÂÃÂ</span><span>{item}</span>
              </div>
            ))}
            <div style={{ marginTop:16, fontSize:13, color:'#F59E0B', fontStyle:'italic' }}>Ã¢ÂÂÃÂÃÂ Situation is live Ã¢ÂÂÃÂÃÂ outcome subject to Boro BistroÃ¢ÂÂs next infraction</div>
          </div>
          <div style={{ background:'#0D1117', border:'1px solid #21262D', borderRadius:10, padding:24, borderTop:'3px solid #2DD4BF' }}>
            ...
            {['Very large additional outdoor capacity for bar service and social use','Significantly increases peak revenue potential, particularly for events and summer trading','No takeaway licence required Ã¢ÂÂÃÂÃÂ all private land, no public highway involvement','Bar sales extend naturally into the yard without additional licensing complexity','Transforms the venueÃ¢ÂÂs kerb presence on one of London's most visited streets'].map(item => (
              <div key={item} style={{ display:'flex', gap:10, marginBottom:10, fontSize:14, color:'#D1D5DB' }}>
                <span style={{ color:'#2DD4BF', flexShrink:0 }}>Ã¢ÂÂÃÂÃÂ</span><span>{item}</span>
              </div>
            ))}
            <div style={{ marginTop:16, display:'flex', flexDirection:'column', gap:6 }}>
              {['Private land Ã¢ÂÂÃÂÃÂ no licensing barrier to operation','No additional licence required to trade in the yard','High revenue impact Ã¢ÂÂÃÂÃÂ significant uplift in capacity and bar sales'].map(item => (
                <div key={item} style={{ fontSize:13, color:'#2DD4BF', fontStyle:'italic' }}>Ã¢ÂÂÃÂÃÂ {item}</div>
              ))}
            </div>
          </div>
        </div>
        <div style={{ background:'rgba(248,113,113,0.06)', border:'1px solid rgba(248,113,113,0.2)', borderRadius:8, padding:'16px 20px' }}>
          <p style={{ fontSize:13, color:'#9CA3AF', fontStyle:'italic', marginBottom:12 }}>Ã¢ÂÂÃÂÃÂ The yard is not speculative pipeline Ã¢ÂÂÃÂÃÂ it is an active situation. If Boro Bistro commit one further breach of their agreement, the opportunity to take back this space becomes available immediately.</p>
          <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
            {[['Private Land Ã¢ÂÂÃÂÃÂ No Licensing Barrier','#2DD4BF'],['Landlord: Southwark Cathedral','#9CA3AF'],['Conditional on Boro Bistro Breach','#C9A84C']].map(([tag,col]) => (
      ...
      <div style={{ marginBottom:40 }}>
        ...
        <div style={{ background:'rgba(245,158,11,0.08)', border:'1px solid rgba(245,158,11,0.3)', borderRadius:8, padding:'14px 18px', marginBottom:24, fontSize:13, color:'#D1D5DB', lineHeight:1.6 }}>
          <span style={{ color:'#F59E0B' }}>Ã¢ÂÂÃÂÃÂ ÃÂ¯ÃÂ¸ÃÂ</span> Note: The licence is currently held by Plonk Golf Ltd (Co. 10328982). A transfer to No Dice Borough Ltd must be completed before or alongside any variation application. The Designated Premises Supervisor (DPS) appointment will also need to be reviewed at that stage.
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
          <div style={{ background:'#0D1117', border:'1px solid #21262D', borderRadius:10, padding:24, borderTop:'3px solid #4FC3F7' }}>
            ...
            {['Conditions 341, 342 and 343 restrict all activity to 11pmÃ¢ÂÂÃÂÃÂ11:30pm','DJ and late-night events format requires a 1am licence as a minimum','Activity-led venue (not a bar) strengthens the case significantly','Propose a 12-month trial period to reduce authorityÃ¢ÂÂs perceived risk','Zero complaint history in SE1 is the strongest single asset'].map(item => (
              <div key={item} style={{ display:'flex', gap:10, marginBottom:8, fontSize:13, color:'#D1D5DB' }}><span style={{ color:'#4FC3F7', flexShrink:0 }}>Ã¢ÂÂÃÂÃÂ</span><span>{item}</span></div>
            ))}
            <div style={{ marginTop:20 }}>
              {[['To 1am','35Ã¢ÂÂÃÂÃÂ45%',0.4,'#4FC3F7'],['To 2am','20Ã¢ÂÂÃÂÃÂ30%',0.25,'#6B7280']].map(([label,pct,val,col]) => (
          ...
          <div style={{ background:'#0D1117', border:'1px solid #21262D', borderRadius:10, padding:24, borderTop:'3px solid #2DD4BF' }}>
            ...
            {['Condition 340 currently limits alcohol to golf, arcade and board game participants only','Board game players can already drink freely Ã¢ÂÂÃÂÃÂ extending this logic to all activity participants is proportionate and defensible','Proposed wording: any customer using the venueÃ¢ÂÂs activity or games facilities may purchase alcohol','Removes a compliance burden without changing the activity-led character','Strongest application Ã¢ÂÂÃÂÃÂ apply for this one first'].map(item => (
              <div key={item} style={{ display:'flex', gap:10, marginBottom:8, fontSize:13, color:'#D1D5DB' }}><span style={{ color:'#2DD4BF', flexShrink:0 }}>Ã¢ÂÂÃÂÃÂ</span><span>{item}</span></div>
            ))}
            <div style={{ marginTop:20 }}>
              <div style={{ display:'flex', justifyContent:'space-between', fontSize:13, marginBottom:4 }}>
                <span style={{ color:'#2DD4BF' }}>Likelihood</span><span style={{ color:'#9CA3AF' }}>65Ã¢ÂÂÃÂÃÂ75%</span>
              </div>
              <div style={{ height:6, background:'#1A1A1A', borderRadius:3 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
          <div style={{ background:'#0D1117', border:'1px solid #21262D', borderRadius:10, padding:24, borderTop:'3px solid #C9A84C' }}>
            ...
            {['Condition 353 blanket-prohibits all live sports screenings','Licence already includes Indoor Sporting Event as a licensed activity Ã¢ÂÂÃÂÃÂ a useful inconsistency to argue at variation','Not seeking to become a sports bar Ã¢ÂÂÃÂÃÂ international tournaments only (World Cup, Euros, Olympics)','Propose advance notification protocol: inform authority before each screening and maintain a log','Avoids the weekly Premier League concerns that drove the original condition'].map(item => (
              <div key={item} style={{ display:'flex', gap:10, marginBottom:8, fontSize:13, color:'#D1D5DB' }}><span style={{ color:'#C9A84C', flexShrink:0 }}>Ã¢ÂÂÃÂÃÂ</span><span>{item}</span></div>
            ))}
            <div style={{ marginTop:20 }}>
              {[['International only','40Ã¢ÂÂÃÂÃÂ55%',0.48,'#C9A84C'],['Full removal','25Ã¢ÂÂÃÂÃÂ35%',0.3,'#6B7280']].map(([label,pct,val,col]) => (
                <div key={label} style={{ marginBottom:12 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', fontSize:13, marginBottom:4 }}>
          ...
          <div style={{ background:'#0D1117', border:'1px solid #21262D', borderRadius:10, padding:24, borderTop:'3px solid #9CA3AF' }}>
            ...
            {['Apply for the golf-drink link change first Ã¢ÂÂÃÂÃÂ easiest win, builds credibility with the authority','Obtain a formal zero-incident letter from Met Police Southwark and Southwark Council licensing team','Engage a Southwark specialist licensing solicitor before any application is submitted','Pre-consult with the licensing authority and police informally before formal submission','Engage Southwark Cathedral directly Ã¢ÂÂÃÂÃÂ their support on hours would carry exceptional weight as landlord','For hours: propose a 12-month time-limited trial to reduce perceived risk for the authority'].map((item,i) => (
        ...
        <div style={{ background:'rgba(99,102,241,0.06)', border:'1px solid rgba(99,102,241,0.2)', borderLeft:'4px solid #6366F1', borderRadius:8, padding:'16px 20px', marginBottom:16 }}>
          <p style={{ fontSize:13, color:'#9CA3AF', fontStyle:'italic', marginBottom:12 }}>Ã¢ÂÂÃÂÃÂ Do not apply for all four variations simultaneously Ã¢ÂÂÃÂÃÂ this signals a venue seeking to transform its character. Sequence the applications. A clean zero-incident trading record in SE1 is the strongest asset in every application.</p>
          <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
            {[['Remove Golf-Drink Link 65Ã¢ÂÂÃÂÃÂ75%','#2DD4BF'],['Hours to 1am 35Ã¢ÂÂÃÂÃÂ45%','#4FC3F7'],['Sports Screening 40Ã¢ÂÂÃÂÃÂ55%','#C9A84C'],['Hours to 2am 20Ã¢ÂÂÃÂÃÂ30%','#9CA3AF']].map(([tag,col]) => (
              <span key={tag} style={{ fontSize:11, border:`1px solid ${col}`, color:col, borderRadius:4, padding:'3px 10px', letterSpacing:'0.08em', textTransform:'uppercase' }}>{tag}</span>
            ))}
          </div>
        </div>
      ...
      <div style={{ padding:'20px 32px 0', borderBottom:'1px solid rgba(201,168,76,0.12)' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:12 }}>
          <div>
            ...
            <div style={{ fontSize:14, color:'var(--cream-dim)' }}>Venue Information ÃÂÃÂ· Arches B C D And E, Montague Close SE1</div>
          </div>
        </div>
