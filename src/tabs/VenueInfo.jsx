...
function TabLocation() {
  const landmarks = [
    { icon:'🚂', name:'London Bridge Station', type:'Transport', dist:'3 min walk', color:'#4FC3F7' },
    { icon:'🛒', name:'Borough Market', type:'Footfall', dist:'Adjacent', color:'#C9A84C' },
    { icon:'🏙️', name:'The Shard', type:'Landmark', dist:'5 min walk', color:'#2DD4BF' },
    { icon:'🚇', name:'Southwark Station', type:'Transport', dist:'7 min walk', color:'#4FC3F7' },
    { icon:'🚇', name:'Borough Station', type:'Transport', dist:'8 min walk', color:'#4FC3F7' },
    { icon:'🎨', name:'Tate Modern', type:'Cultural', dist:'12 min walk', color:'#2DD4BF' },
    { icon:'🏦', name:'City of London', type:'Business', dist:'10 min walk', color:'#4FC3F7' },
    { icon:'🌉', name:'South Bank', type:'Tourism', dist:'10 min walk', color:'#4FC3F7' },
  ]
  const transport = [
  ...
  const topCards = [
    { icon:'🧠', name:'London Bridge', detail:'3 min walk', sub:'Jubilee · Northern · Southeastern', color:'#4FC3F7' },
    { icon:'👥', name:'Borough Market', detail:'Adjacent', sub:'15–20M annual visitors · One of Europe’s most visited food destinations', color:'#C9A84C' },
    { icon:'📈', name:'Tourist Destination', detail:'South Bank', sub:'London-themed golf course ties to Tate, Shard & South Bank day outs', color:'#2DD4BF' },
    { icon:'🖼️', name:'South Bank', detail:'10 min walk', sub:'Tourism', color:'#4FC3F7' },
  ]
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
      ...
      <div style={{ background:'var(--ink-2)', border:'1px solid rgba(201,168,76,0.12)', borderRadius:10, padding:20 }}>
        ...
        {[['Mon–Fri','11:00 – 23:30'],['Saturday','11:00 – 23:30'],['Sunday','11:00 – 23:30']].map(([l,v]) => <Row key={l} label={l} value={v} />)}
      </div>
    </div>
  )
}
function TabFloorPlan() {
  const specs = [
    { label:'UPSTAIRS', badge:null, value:'400m²', color:'#4FC3F7', note:null },
    { label:'DOWNSTAIRS', badge:'SPACE CURRENTLY USED FOR STORES, CELLAR & KITCHEN', value:'300m²', color:'#C9A84C', note:null },
    { label:'LICENSE', badge:null, value:'11pm', color:'#2DD4BF', note:null },
    { label:'CAPACITY', badge:null, value:'120', color:'#4FC3F7', note:null },
    { label:'TOILETS', badge:null, value:'2', color:'#C9A84C', note:null },
    { label:'FIRE EXITS', badge:null, value:'3', color:'#2DD4BF', note:null },
    { label:'PLANNING', badge:null, value:'E Class', color:'#C9A84C', note:null },
  ]
  return (
    <div style={{ display:'grid', gridTemplateColumns:'1.4fr 1fr', gap:20, alignItems:'start' }}>
      <div style={{ background:'var(--ink-2)', border:'1px solid rgba(201,168,76,0.12)', borderRadius:10, padding:16 }}>
        ...
        <img src="/floorplan_1.png" alt="Venue floor plan" style={{ width:'100%', borderRadius:6, objectFit:'contain', maxHeight:560, display:'block' }} />
      </div>
...
function TabLicence() {
  const a1 = [
    ...
    {r:'485',t:'No irresponsible promotions â including activities encouraging individuals to drink within time limits, provision of unlimited quantities of alcohol at a fixed or discounted fee, or dispensing alcohol directly into another personâs mouth.'},
    ...
    {r:'489',t:'Mandatory measures. Beer/cider: Â½ pint; Gin/rum/vodka/whisky: 25ml or 35ml; Still wine: 125ml.'},
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:0 }}>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12 }}>
        <div style={cs}>
          <STitle>Part 1 â Premises Details</STitle>
          <Row label="Premises Licence" value="Licensing Act 2003 Â· Southwark Council" />
        ...
        <div style={cs}>
          <STitle>Part 2 â Licence Holder</STitle>
      ...
      <div style={cs}>
        <STitle>Opening Hours â 07:00 to 23:30 all days</STitle>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:4, marginBottom:12 }}>
          {days.map(d=>(
            <div key={d} style={{ textAlign:'center' }}>
              ...
              <div style={{ fontSize:10, color:'var(--gold-dim)', margin:'2px 0' }}>â</div>
              <div style={{ fontSize:11, color:'var(--cream)', background:'var(--ink-3)', borderRadius:4, padding:'3px 2px' }}>23:30</div>
            </div>
          ))}
        </div>
        <STitle>Licensed Activity Hours â 11:00 to 23:00 all days</STitle>
        {['Films â Indoors','Indoor Sporting Event','Alcohol â On Premises'].map(act=>(
          <div key={act} style={{ fontSize:12, color:'var(--cream-dim)', padding:'4px 0', borderBottom:'1px solid rgba(255,255,255,0.04)' }}>{act}</div>
        ))}
      </div>
      <div style={cs}>
        <STitle>Annex 1 â Mandatory Conditions</STitle>
        {a1.map(c=>(
      ...
      <div style={cs}>
        <STitle>Annex 2 â Operating Schedule Conditions</STitle>
        {a2.map(c=>(
          <div key={c.r} style={{ display:'flex', gap:12, padding:'8px 0', borderBottom:'1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ fontSize:12, color:'var(--gold)', minWidth:36, flexShrink:0, fontWeight:600 }}>{c.r}</div>
            <div style={{ fontSize:12, color:'var(--cream-dim)', lineHeight:1.6 }}>{c.t}</div>
...
function TabDevelopment() {
  const Arr = ({c}) => <span style={{ color:c, flexShrink:0 }}>â</span>
      ...
      <div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
          <div style={card('#C9A84C')}>
            ...
            {['200mÂ² of basement space directly below the bar and arcade arch','Available for development now â no structural barriers','Proposed uses: karaoke rooms Â· listening bar Â· private hire games spaces','High-margin private hire and events revenue potential','Extends the venue offer without expanding the footprint above ground'].map((item,i) => <div key={i} style={{ display:'flex', gap:10, marginBottom:10, fontSize:14, color:'#D1D5DB' }}><Arr c="#C9A84C" /><span>{item}</span></div>)}
          </div>
          <div style={card('#4FC3F7')}>
            ...
            {['300mÂ² of developable basement beneath the full Borough venue','Rights to sublet and carry out works are held within the lease','Structural costs covered by the landlord and building insurance policy','Fit-out to be carried out by No Dice Borough Ltd','A further 100mÂ² tunnel opens under the final golf arch once TfL installs a new staircase â at that point the entire basement connects for public use','Full basement development unlocks a significant additional revenue floor'].map((item,i) => <div key={i} style={{ display:'flex', gap:10, marginBottom:10, fontSize:14, color:'#D1D5DB' }}><Arr c="#4FC3F7" /><span>{item}</span></div>)}
          </div>
        </div>
        <div style={{ background:'rgba(45,212,191,0.06)', border:'1px solid rgba(45,212,191,0.2)', borderRadius:8, padding:'16px 20px' }}>
          <p style={{ fontSize:13, color:'#9CA3AF', fontStyle:'italic', marginBottom:12 }}>The basement represents significant embedded upside â it is not speculative. The rights exist, the space exists, and the structural costs are not borne by No Dice Borough Ltd.</p>
          <Tag label="Lease-Backed Rights" color="#4FC3F7" /><Tag label="Structural Costs: Landlord" color="#2DD4BF" /><Tag label="400mÂ² Total Potential" color="#C9A84C" />
        </div>
      </div>
      <div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
          <div style={card('#F87171')}>
            ...
            <h3 style={{ fontSize:18, fontWeight:700, color:'#fff', marginBottom:16 }}>Boro Bistro â Final Warning</h3>
            ...
            <div style={{ marginTop:16, fontSize:13, color:'#F59E0B', fontStyle:'italic' }}>Situation is live â outcome subject to Boro Bistroâs next infraction</div>
          </div>
          <div style={card('#2DD4BF')}>
            ...
            {['Very large additional outdoor capacity for bar service and social use','Significantly increases peak revenue potential, particularly for events and summer trading','No takeaway licence required â all private land, no public highway involvement','Bar sales extend naturally into the yard without additional licensing complexity','Transforms the kerb presence on one of Londonâs most visited streets'].map((item,i) => <div key={i} style={{ display:'flex', gap:10, marginBottom:10, fontSize:14, color:'#D1D5DB' }}><Arr c="#2DD4BF" /><span>{item}</span></div>)}
            <div style={{ marginTop:16, display:'flex', flexDirection:'column', gap:6 }}>
              {['Private land â no licensing barrier to operation','No additional licence required to trade in the yard','High revenue impact â significant uplift in capacity and bar sales'].map((item,i) => <div key={i} style={{ fontSize:13, color:'#2DD4BF', fontStyle:'italic' }}>â {item}</div>)}
            </div>
          </div>
        </div>
        <div style={{ background:'rgba(248,113,113,0.06)', border:'1px solid rgba(248,113,113,0.2)', borderRadius:8, padding:'16px 20px' }}>
          <p style={{ fontSize:13, color:'#9CA3AF', fontStyle:'italic', marginBottom:12 }}>The yard is not speculative pipeline â it is an active situation. If Boro Bistro commit one further breach of their agreement, the opportunity to take back this space becomes available immediately.</p>
          <Tag label="Private Land â No Licensing Barrier" color="#2DD4BF" /><Tag label="Landlord: Southwark Cathedral" color="#9CA3AF" /><Tag label="Conditional on Boro Bistro Breach" color="#C9A84C" />
        </div>
      </div>
      <div>
        ...
        <div style={{ background:'rgba(245,158,11,0.08)', border:'1px solid rgba(245,158,11,0.3)', borderRadius:8, padding:'14px 18px', marginBottom:24, fontSize:13, color:'#D1D5DB', lineHeight:1.6 }}>
          <span style={{ color:'#F59E0B' }}>â ï¸ </span>Note: The licence is currently held by Plonk Golf Ltd (Co. 10328982). A transfer to No Dice Borough Ltd must be completed before or alongside any variation application. The DPS appointment will also need to be reviewed at that stage.
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
          <div style={card('#4FC3F7')}>
            ...
            {['Conditions 341, 342 and 343 restrict all activity to 11pmâ11:30pm','DJ and late-night events format requires a 1am licence as a minimum','Activity-led venue (not a bar) strengthens the case significantly','Propose a 12-month trial period to reduce the perceived risk','Zero complaint history in SE1 is the strongest single asset'].map((item,i) => <div key={i} style={{ display:'flex', gap:10, marginBottom:8, fontSize:13, color:'#D1D5DB' }}><Arr c="#4FC3F7" /><span>{item}</span></div>)}
            <div style={{ marginTop:20 }}><Bar label="To 1am" pct="35â45%" val={0.4} color="#4FC3F7" /><Bar label="To 2am" pct="20â30%" val={0.25} color="#6B7280" /></div>
          </div>
          <div style={card('#2DD4BF')}>
            ...
            {['Condition 340 currently limits alcohol to golf, arcade and board game participants only','Board game players can already drink freely â extending this logic to all activity participants is proportionate and defensible','Proposed wording: any customer using the venue activity or games facilities may purchase alcohol','Removes a compliance burden without changing the activity-led character','Strongest application â apply for this one first'].map((item,i) => <div key={i} style={{ display:'flex', gap:10, marginBottom:8, fontSize:13, color:'#D1D5DB' }}><Arr c="#2DD4BF" /><span>{item}</span></div>)}
            <div style={{ marginTop:20 }}><Bar label="Likelihood" pct="65â75%" val={0.7} color="#2DD4BF" /></div>
          </div>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
          <div style={card('#C9A84C')}>
            ...
            {['Condition 353 blanket-prohibits all live sports screenings','Licence already includes Indoor Sporting Event as a licensed activity â a useful inconsistency to argue at variation','Not seeking to become a sports bar â international tournaments only (World Cup, Euros, Olympics)','Propose advance notification protocol: inform authority before each screening and maintain a log','Avoids the weekly Premier League concerns that drove the original condition'].map((item,i) => <div key={i} style={{ display:'flex', gap:10, marginBottom:8, fontSize:13, color:'#D1D5DB' }}><Arr c="#C9A84C" /><span>{item}</span></div>)}
            <div style={{ marginTop:20 }}><Bar label="International only" pct="40â55%" val={0.48} color="#C9A84C" /><Bar label="Full removal" pct="25â35%" val={0.3} color="#6B7280" /></div>
          </div>
          <div style={card('#9CA3AF')}>
            ...
            {['Apply for the golf-drink link change first â easiest win, builds credibility with the authority','Obtain a formal zero-incident letter from Met Police Southwark and Southwark Council licensing team','Engage a Southwark specialist licensing solicitor before any application is submitted','Pre-consult with the licensing authority and police informally before formal submission','Engage Southwark Cathedral directly â their support on hours would carry exceptional weight as landlord','For hours: propose a 12-month time-limited trial to reduce perceived risk for the authority'].map((item,i) => (
        ...
        <div style={{ background:'rgba(99,102,241,0.06)', border:'1px solid rgba(99,102,241,0.2)', borderLeft:'4px solid #6366F1', borderRadius:8, padding:'16px 20px', marginBottom:16 }}>
          <p style={{ fontSize:13, color:'#9CA3AF', fontStyle:'italic', marginBottom:12 }}>Do not apply for all four variations simultaneously â this signals a venue seeking to transform its character. Sequence the applications. A clean zero-incident trading record in SE1 is the strongest asset in every application.</p>
          <Tag label="Remove Golf-Drink Link 65â75%" color="#2DD4BF" /><Tag label="Hours to 1am 35â45%" color="#4FC3F7" /><Tag label="Sports Screening 40â55%" color="#C9A84C" /><Tag label="Hours to 2am 20â30%" color="#9CA3AF" />
        </div>
