...
function MiniBar({data,keyR,keyE,maxV}){
  return(
    <div>
      ...
      <div style={{display:'flex',gap:2,marginTop:3}}>{data.map((d,i)=><div key={i} style={{flex:1,fontSize:7,color:'var(--cream-dim)',textAlign:'center'}}>{d.m}</div>)}</div>
    </div>
  )
}
...
function STitle({children}){
  return <div style={{fontSize:9,letterSpacing:'0.1em',textTransform:'uppercase',color:'var(--gold-dim)',marginBottom:10}}>{children}</div>
}
function Row({label,value,gold}){
  return(
    <div style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid rgba(255,255,255,0.05)'}}>
      <span style={{fontSize:10,color:'var(--cream-dim)'}}>{label}</span>
      <span style={{fontSize:10,color:gold?'var(--gold)':'var(--cream)'}}>{value}</span>
    </div>
  )
}
    ...
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12}}>
        {[['Investment Ask',fmt(FIG.investment),'for 49% equity'],['Investor Equity','49.00%','51% retained by founder'],['FY2025 Revenue',fmt(FIG.rev2025),'Verified financial model'],['Forecast Y1',fmt(FIG.forecastRev),'+15.0% on prior year']].map(([l,v,s])=>(
          <Card key={l}><STitle>{l}</STitle><div style={{fontSize:22,color:'var(--gold)',marginBottom:3}}>{v}</div><div style={{fontSize:10,color:'var(--cream-dim)'}}>{s}</div></Card>
        ))}
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
        <Card><STitle>Revenue Split</STitle>
          <div style={{display:'flex',alignItems:'center',gap:14}}>
            ...
            <div style={{flex:1}}>{INCOME_SPLIT.map(s=>(<div key={s.n} style={{display:'flex',justifyContent:'space-between',padding:'3px 0',borderBottom:'1px solid rgba(255,255,255,0.05)'}}><div style={{display:'flex',alignItems:'center',gap:6}}><div style={{width:8,height:8,borderRadius:2,background:s.col,flexShrink:0}} /><span style={{fontSize:10,color:'var(--cream)'}}>{s.n}</span></div><span style={{fontSize:10,color:'var(--gold)'}}>{s.pct}%</span></div>))}</div>
          </div>
        </Card>
        <Card><STitle>Investment Summary</STitle>
          ...
          <div style={{marginTop:10,padding:10,background:'rgba(201,168,76,0.06)',borderRadius:7,border:'1px solid rgba(201,168,76,0.15)'}}>
            <div style={{fontSize:9,color:'var(--gold-dim)',marginBottom:4}}>BASE CASE RETURNS</div>
            <div style={{fontSize:24,color:'var(--gold)'}}>{pct(calcReturns(FIG.forecastRev).coc)}</div>
            <div style={{fontSize:10,color:'var(--cream-dim)'}}>Cash-on-cash return in Year 1</div>
          </div>
        </Card>
      </div>
    </div>
    ...
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12}}>
        {[['2025 Revenue',fmt(FIG.rev2025),'Verified actuals'],['2025 EBITDA',fmt(FIG.ebitda2025),'12.4% margin'],['2025 Net Profit',fmt(FIG.profit2025),'15.0% margin'],['Wages % Revenue','32.7%','£242,370 ÷ £741,644']].map(([l,v,s])=>(
          <Card key={l}><STitle>{l}</STitle><div style={{fontSize:20,color:'var(--gold)',marginBottom:3}}>{v}</div><div style={{fontSize:10,color:'var(--cream-dim)'}}>{s}</div></Card>
        ))}
      </div>
      <div style={{display:'flex',gap:4,borderBottom:'1px solid rgba(201,168,76,0.1)'}}>
        {[['revenue','Revenue'],['costs','Cost Split'],['income','Income Split']].map(([k,l])=>(
          <button key={k} onClick={()=>setView(k)} style={{padding:'6px 14px',fontSize:10,cursor:'pointer',border:'none',background:'transparent',borderBottom:`2px solid ${view===k?'var(--gold)':'transparent'}`,color:view===k?'var(--gold)':'var(--cream-dim)',transition:'all 0.15s'}}>{l}</button>
        ))}
      </div>
      {view==='revenue'&&<div style={{display:'flex',flexDirection:'column',gap:12}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:8}}>{MON.map(m=>(<Card key={m.m}><div style={{fontSize:9,color:'var(--cream-dim)',marginBottom:3}}>{m.m}</div><div style={{fontSize:11,color:'var(--gold)'}}>{fmtK(m.r)}</div><div style={{fontSize:9,color:'var(--teal)'}}>E: {fmtK(m.e)}</div></Card>))}</div>
      </div>}
      {view==='costs'&&<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
        <Card><STitle>Cost Structure 2025</STitle>
          <div style={{display:'flex',alignItems:'center',gap:12}}><DonutSVG data={COSTS_SPLIT} total={630467} />
            <div style={{flex:1}}>{COSTS_SPLIT.map(s=>(<div key={s.n} style={{padding:'3px 0',borderBottom:'1px solid rgba(255,255,255,0.05)'}}><div style={{display:'flex',justifyContent:'space-between'}}><div style={{display:'flex',alignItems:'center',gap:5}}><div style={{width:7,height:7,borderRadius:2,background:s.col}} /><span style={{fontSize:10,color:'var(--cream)'}}>{s.n}</span></div><span style={{fontSize:9,color:'var(--cream-dim)'}}>{s.pct}%</span></div></div>))}</div>
          </div>
        </Card>
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          <Card><STitle>Total Cost Base</STitle><div style={{fontSize:24,color:'var(--cream)'}}>£630,467</div><div style={{fontSize:10,color:'var(--cream-dim)'}}>85.0% of revenue</div></Card>
      ...
      {view==='income'&&<Card><STitle>Income by Source 2025</STitle>
        <div style={{display:'flex',alignItems:'center',gap:16}}><DonutSVG data={INCOME_SPLIT} total={FIG.rev2025} size={130} />
          <div style={{flex:1}}>{INCOME_SPLIT.map(s=>(<div key={s.n} style={{padding:'6px 0',borderBottom:'1px solid rgba(255,255,255,0.05)'}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:3}}><div style={{display:'flex',alignItems:'center',gap:7}}><div style={{width:9,height:9,borderRadius:2,background:s.col}} /><span style={{fontSize:11,color:'var(--cream)'}}>{s.n}</span></div><div style={{display:'flex',gap:10}}><span style={{fontSize:10,color:'var(--cream-dim)'}}>{s.pct}%</span><span style={{fontSize:11,color:'var(--gold)'}}>{fmt(FIG.rev2025*s.pct/100)}</span></div></div><div style={{height:4,background:'var(--ink-3)',borderRadius:2}}><div style={{height:'100%',width:`${s.pct}%`,background:s.col,borderRadius:2}} /></div></div>))}</div>
        </div>
      </Card>}
    </div>
  )
    ...
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      <Card highlight>
        ...
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:5,fontSize:12}}>
          ...
          <span style={{color:'var(--gold)',fontSize:20}}>{fmt(inv)}</span>
        </div>
        ...
        <div style={{display:'flex',justifyContent:'space-between',fontSize:9,color:'var(--gold-dim)',marginBottom:14}}><span>£10k</span><span>£100k</span><span>£200k</span></div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:12}}>
          {[['Ownership',pct(eq),'var(--cream)'],['Preferred',fmt(pref),'var(--gold)'],['Dividend',fmt(div2),'var(--cream)'],['Total Year 1',fmt(total),'var(--gold)']].map(([l,v,c])=>(
            <div key={l} style={{textAlign:'center',padding:10,background:'var(--ink-3)',borderRadius:8}}>
              <div style={{fontSize:9,color:'var(--cream-dim)',marginBottom:4}}>{l}</div>
              <div style={{fontSize:16,color:c}}>{v}</div>
            </div>
          ))}
        </div>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
          <div style={{padding:10,background:'var(--ink-3)',borderRadius:8}}><div style={{fontSize:9,color:'var(--cream-dim)',marginBottom:3}}>Cash-on-Cash</div><div style={{fontSize:18,color:'var(--gold)'}}>{pct(coc)}</div></div>
          <div style={{padding:10,background:'var(--ink-3)',borderRadius:8}}><div style={{fontSize:9,color:'var(--cream-dim)',marginBottom:3}}>Payback Period</div><div style={{fontSize:18,color:'var(--gold)'}}>{pb.toFixed(2)} years</div></div>
        </div>
        <div style={{marginTop:10,padding:10,background:isA?'rgba(45,212,191,0.06)':'rgba(255,255,255,0.03)',border:`1px solid ${isA?'rgba(45,212,191,0.2)':'rgba(255,255,255,0.06)'}`,borderRadius:7,fontSize:10,color:'var(--cream-dim)'}}>
    ...
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      <div style={{display:'flex',gap:5}}>
        {Object.entries(SCENARIOS).map(([k,sc2])=>(
          <button key={k} onClick={()=>setSc(k)} style={{padding:'6px 14px',fontSize:10,borderRadius:6,cursor:'pointer',background:sc===k?`${sc2.col}22`:'transparent',border:`1px solid ${sc===k?sc2.col:'rgba(201,168,76,0.25)'}`,color:sc===k?sc2.col:'var(--cream-dim)',transition:'all 0.15s'}}>{sc2.l}</button>
        ))}
      </div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
        <Card><STitle>Distribution Waterfall</STitle>
          {[['Operating Profit',r.profit,'#1565C0',''],['Less: Preferred Return',-FIG.investment*FIG.prefRate,'#B71C1C','8% × £150k — investor first'],['Less: A-Share Priority',-FIG.aShare,'#E67E22','Founder entity priority'],['Remaining Pool',r.rem,'#0D9488','Available for equity split'],['Investor Dividend (49%)',r.iDiv,'#C9A84C','49% × remaining pool'],['Founder Dividend (51%)',r.fDiv,'#4A5568','51% × remaining pool']].map(([l,v,c,n])=>(
            <div key={l} style={{display:'flex',alignItems:'center',gap:9,padding:'8px 0',borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
              ...
              <div style={{flex:1}}><div style={{fontSize:10,color:'var(--cream)'}}>{l}</div>{n&&<div style={{fontSize:8,color:'var(--cream-dim)'}}>{n}</div>}</div>
              <div style={{fontSize:14,color:v<0?'#E53935':c}}>{v<0?'−':''}{fmt(Math.abs(v))}</div>
            </div>
          ))}
        </Card>
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          <Card highlight>
            <div style={{fontSize:8,color:'var(--gold-dim)',marginBottom:5,textTransform:'uppercase',letterSpacing:'0.1em'}}>Total Investor Return · {s.l}</div>
            <div style={{fontSize:32,color:'var(--gold)',marginBottom:3}}>{fmt(r.total)}</div>
            <div style={{fontSize:10,color:'var(--cream-dim)'}}>Preferred {fmt(FIG.investment*FIG.prefRate)} + Dividend {fmt(r.iDiv)}</div>
          </Card>
          <Card>
          ...
          <div style={{fontSize:10,color:'var(--cream-dim)',lineHeight:1.6}}>Cash-flow driven — no exit required. Investor receives full return from Year 1 trading distributions only.</div>
        </div>
      </div>
    </div>
  )
    ...
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      <Card highlight>
        ...
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:5,fontSize:12}}>
          ...
          <span style={{color:'var(--gold)',fontSize:18}}>{growth>0?'+':''}{growth}%</span>
        </div>
        ...
        <div style={{display:'flex',justifyContent:'space-between',fontSize:9,color:'var(--gold-dim)',marginBottom:14}}><span>−10%</span><span>+15% base</span><span>+40%</span></div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10}}>
          {[['Forecast Revenue',fmt(rev)],['Forecast Profit',fmt(r.profit)],['Investor Return',fmt(r.total)],['Cash-on-Cash',pct(r.coc)]].map(([l,v])=>(
            <div key={l} style={{textAlign:'center',padding:10,background:'var(--ink-3)',borderRadius:8}}>
              <div style={{fontSize:9,color:'var(--cream-dim)',marginBottom:4,textTransform:'uppercase',letterSpacing:'0.06em'}}>{l}</div>
              <div style={{fontSize:16,color:'var(--gold)'}}>{v}</div>
            </div>
          ))}
        </div>
      </Card>
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12}}>
        {Object.entries(SCENARIOS).map(([k,s])=>{
          ...
          return(
            <Card key={k}>
              <div style={{fontSize:9,textTransform:'uppercase',letterSpacing:'0.1em',color:s.col,marginBottom:8}}>{s.l}</div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:10,marginBottom:16}}>
          {[['~5.3×','UK Mid-Market Average','Above this deal','var(--cream-dim)'],['~4.1×','Hospitality & Leisure Sector Avg','Broadly in line','var(--cream-dim)'],['~2–4×','Small Single-Site Venues','In range','var(--cream-dim)'],['~2–3×','Distressed Asset Range','Above — priced for risk','var(--cream-dim)'],['1.70×','No Dice Borough — This Deal','★ Entry point','var(--gold)']].map(([v,l,n,c])=>(
            <div key={l} style={{padding:'12px 10px',borderRadius:8,textAlign:'center',background:c==='var(--gold)'?'rgba(201,168,76,0.1)':'var(--ink-3)',border:`1px solid ${c==='var(--gold)'?'rgba(201,168,76,0.4)':'rgba(255,255,255,0.06)'}`}}>
              <div style={{fontSize:20,color:c,marginBottom:5}}>{v}</div>
              <div style={{fontSize:8,color:'var(--cream-dim)',marginBottom:4,lineHeight:1.4}}>{l}</div>
              <div style={{fontSize:8,color:c==='var(--gold)'?'var(--gold)':'rgba(184,176,160,0.5)'}}>{n}</div>
            </div>
          ))}
        </div>
        <div style={{padding:'12px 14px',background:'rgba(201,168,76,0.06)',border:'1px solid rgba(201,168,76,0.2)',borderRadius:8,fontSize:11,color:'var(--cream-dim)',lineHeight:1.6,marginBottom:14}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
          {[['✓ Not a multiple expansion bet','Returns driven by operating cash flow'],['✓ Cash-yielding from Day 1','Distributions begin at end of Year 1'],['✓ All 3 scenarios positive','Conservative through Optimistic']].map(([t,s])=>(
            <div key={t} style={{padding:10,background:'rgba(45,212,191,0.06)',border:'1px solid rgba(45,212,191,0.15)',borderRadius:7}}>
              <div style={{fontSize:10,color:'var(--teal)',marginBottom:3}}>{t}</div>
              <div style={{fontSize:9,color:'var(--cream-dim)'}}>{s}</div>
            </div>
          ))}
        </div>
      </Card>
      <Card>
        ...
        {[['Employer NICs rose 13.8% → 15% (April 2025)','Labour cost increases are built into the forecast model — not hidden'],['National Minimum Wage up 6.7% to £12.21/hr (April 2025)','Wage inflation modelled at 2025 actual base — no optimistic assumption'],['Business rates relief cut from 75% → 40% (2025/26)','Cost environment is baked in — not a pre-cost-shock baseline'],['UK hospitality recording ~2 site closures per day (2025)','Sector pressure creates acquisition opportunity at realistic pricing'],["Consumer behaviour shifting toward experience-led, low-alcohol spend","No Dice Borough's experience model directly aligns with this shift"],['PE firms cautious on single-country consumer exposure','Smaller investor opportunity — less institutional competition']].map(([issue,response])=>(
          <div key={issue} style={{padding:'8px 0',borderBottom:'1px solid rgba(255,255,255,0.05)'}}>
            <div style={{fontSize:10,color:'var(--cream)',marginBottom:2}}>▪ {issue}</div>
            <div style={{fontSize:10,color:'var(--teal)',paddingLeft:12}}>→ {response}</div>
          </div>
        ))}
      </Card>
      <Card>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
          {[['&#x1F4C8;','Proven Revenue Base',fmt(FIG.rev2025)+' verified 2025 actuals. Not a projection.'],['&#x1F4CD;','Prime London Location','Borough Market SE1: top footfall destination. Highest avg revenue growth in East/Central London.'],['&#x1F3B1;','Experience-Led Format','Pool, board games, mini golf, DJ nights. Fastest-growing hospitality sub-sector.'],['&#x1F4B0;','Multiple Revenue Streams','Bar + activity pricing + events + corporate hire. Less dependent on drink-only margins.'],['&#x2122;&#xFE0F;','Brand IP Acquired','Plonk trading name, customer data and goodwill purchased. Not starting from zero.'],['&#x1F4E3;','Google Ads Engine','£0.32 CPC and verified ROAS from 2025 GA4 data. Scalable with proven unit economics.']].map(([icon,title,body])=>(
            <div key={title} style={{background:'var(--ink-3)',borderRadius:8,padding:14}}>
              <div style={{fontSize:16,marginBottom:6}} dangerouslySetInnerHTML={{__html:icon}} />
              <div style={{fontSize:11,color:'var(--cream)',fontWeight:500,marginBottom:4}}>{title}</div>
              <div style={{fontSize:9,color:'var(--cream-dim)',lineHeight:1.5}}>{body}</div>
            </div>
          ))}
        </div>
      </Card>
    ...
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12}}>
        {[['Total Wage Bill 2025','£242,370','Monthly P&L'],['Total Hours','10,043 hrs','23 employees'],['Total Shifts','1,283 shifts','Avg 7.8 hrs/shift'],['Wages % Revenue','32.7%','£242,370 ÷ £741,644']].map(([l,v,s])=>(
          <Card key={l}><STitle>{l}</STitle><div style={{fontSize:18,color:'var(--gold)',marginBottom:3}}>{v}</div><div style={{fontSize:9,color:'var(--cream-dim)'}}>{s}</div></Card>
        ))}
      </div>
      <Card>
        ...
        <div style={{display:'flex',gap:3,marginBottom:6}}>{MON_W.map(m=><div key={m.m} style={{flex:1,fontSize:7,color:'var(--cream-dim)',textAlign:'center'}}>{m.m}</div>)}</div>
      ...
      <Card highlight>
        ...
        <div style={{display:'flex',justifyContent:'space-between',fontSize:9,color:'var(--gold-dim)',marginBottom:12}}><span>−20%</span><span>0%</span><span>+20%</span></div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
          {[['Projected 2026 Wages',fmt(242370+delta)],['Delta vs 2025',`${delta>=0?'+':''}${fmt(delta)}`],['Wages % Forecast',((totalWages/FIG.forecastRev)*100).toFixed(1)+'%']].map(([l,v])=>(
            <div key={l} style={{textAlign:'center',padding:10,background:'var(--ink-3)',borderRadius:8}}>
              <div style={{fontSize:9,color:'var(--cream-dim)',marginBottom:3}}>{l}</div>
              <div style={{fontSize:15,color:'var(--gold)'}}>{v}</div>
            </div>
          ))}
        </div>
      </Card>
          ...
          return(
            <Card key={w.role}>
              <div style={{display:'grid',gridTemplateColumns:'1.5fr 1fr 1fr 1fr 1fr',gap:8,alignItems:'center'}}>
                <div><div style={{fontSize:12,color:'var(--cream)',marginBottom:3}}>{w.role}</div><div style={{display:'flex',alignItems:'center',gap:5}}><div style={{width:7,height:7,borderRadius:2,background:w.col}} /><span style={{fontSize:9,color:'var(--cream-dim)'}}>{w.hrs.toLocaleString()} hrs/yr</span></div></div>
                <div style={{textAlign:'right'}}><div style={{fontSize:8,color:'var(--cream-dim)'}}>Rate/hr</div><div style={{fontSize:16,color:'var(--gold)'}}>&#x00A3;{effRate.toFixed(2)}</div></div>
                <div style={{textAlign:'right'}}><div style={{fontSize:8,color:'var(--cream-dim)'}}>Annual</div><div style={{fontSize:13,color:'var(--cream)'}}>{fmt(annualCost)}</div></div>
                <div style={{textAlign:'right'}}><div style={{fontSize:8,color:'var(--cream-dim)'}}>Salary equiv</div><div style={{fontSize:11,color:'var(--cream-dim)'}}>&#x00A3;{Math.round(effRate*40*52).toLocaleString()}</div></div>
                <div style={{textAlign:'right'}}><div style={{fontSize:8,color:'var(--cream-dim)'}}>vs NMW</div><div style={{fontSize:11,color:effRate>=w.min?'#2DD4BF':'#E53935'}}>{effRate>=w.min?'+':''}&#x00A3;{(effRate-w.min).toFixed(2)}</div></div>
              </div>
            </Card>
          )
        })}
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12}}>
          {[['Reduce Jan/Feb shifts by 20%','Saves ~£2,700/yr','Jan: 30.6% → ~24% of revenue','Fewer doubles, shorter shifts in quiet months.'],['Shift Supervisor hours to Bar Staff rate','Saves ~£1,200/yr','Apr–Jul Supervisor cost £19,921','Review whether full Supervisor coverage needed on quiet weekdays.'],['Staff December to capacity','+£3,000 wages · more revenue captured','Dec 2025: 88 shifts · £121k rev','Adding 27 shifts captures significantly more peak revenue.']].map(([t,s,d,b])=>(
            <div key={t} style={{background:'var(--ink-3)',borderRadius:8,padding:14}}>
              <div style={{fontSize:11,color:'var(--cream)',fontWeight:500,marginBottom:4}}>{t}</div>
              <div style={{fontSize:10,color:'var(--gold)',marginBottom:6}}>{s}</div>
              <div style={{fontSize:9,color:'var(--cream-dim)',marginBottom:4}}>{d}</div>
              <div style={{fontSize:9,color:'var(--cream-dim)',lineHeight:1.5}}>{b}</div>
            </div>
          ))}
        </div>
      </Card>
      <div style={{fontSize:9,color:'var(--cream-dim)',lineHeight:1.5}}>NMW April 2025: £12.21/hr · All 2026 base rates above NMW · Rate hierarchy: Bar Staff £13.85 &lt; Supervisor £14.35 &lt; Asst Manager £15.38 &lt; Manager £18.00</div>
    </div>
  )
}
    ...
    <div style={{minHeight:'100%',background:'var(--ink)',color:'var(--cream)'}}>
      <div style={{padding:'20px 32px 0',borderBottom:'1px solid rgba(201,168,76,0.12)'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:12}}>
          <div>
            <div style={{fontSize:10,color:'var(--gold)',letterSpacing:'0.15em',textTransform:'uppercase',marginBottom:4}}>No Dice Borough Ltd</div>
            <div style={{fontSize:13,color:'var(--cream-dim)'}}>Business Explorer · Borough Market SE1</div>
          </div>
          <div style={{textAlign:'right'}}>
            <div style={{fontSize:9,color:'var(--gold-dim)',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:2}}>Investment Ask</div>
            <div style={{fontSize:22,color:'var(--gold)'}}>&#x00A3;150,000</div>
            <div style={{fontSize:10,color:'var(--cream-dim)'}}>for 49% equity</div>
          </div>
        </div>
      ...
      <div style={{padding:'8px 32px',borderTop:'1px solid rgba(201,168,76,0.08)',fontSize:9,color:'var(--gold-dim)',textAlign:'center'}}>Business Explorer · No Dice Borough LTD · All figures from verified financial model</div>
    </div>
  )
            }
