...
export default function UseOfFunds() {
  const funds = [
    { label:'Rent Deposit (3 months)', color:'#8B5CF6', pct:38.7, amount:27078, vat:'inc VAT', icon:'🏠', desc:'Security deposit held by landlord covering May, Jun, Jul 2026.' },
    { label:'Hardware from Liquidators', color:'#4FC3F7', pct:21.4, amount:15000, vat:'inc VAT', icon:'🔧', desc:'Physical bar & kitchen equipment — purchased at liquidation pricing. Operational on Day 1.' },
    { label:'IP License Fee', color:'#C9A84C', pct:17.1, amount:12000, vat:'inc VAT', icon:'⭐', desc:'Brand + gaming IP licence — was a £72,000 inc VAT Plonk IP & Goodwill purchase under the old deal.' },
    { label:'Stock & Supplier Restart', color:'#2DD4BF', pct:11.4, amount:8000, vat:'inc VAT', icon:'📦', desc:'Opening stock, supplier agreements and software subscriptions to trade from Day 1.' },
    { label:'Working Capital Buffer', color:'#6B7280', pct:11.3, amount:7922, vat:null, icon:'💼', desc:'Staged into business per cash flow model. Covers early trading before revenue covers costs.' },
  ]
  ...
  return (
    <div style={{ maxWidth:1100, margin:'0 auto', padding:'0 4px' }}>
      <div style={{ marginBottom:24 }}>
        ...
        <h2 className="serif" style={{ fontSize:'clamp(2rem, 4vw, 3rem)', color:'var(--cream)', marginBottom:8 }}>Where Your £70,000 Goes</h2>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginBottom:20 }}>
        <div style={{ background:'#0D1117', border:'1px solid #21262D', borderRadius:10, padding:24 }}>
          ...
          <div style={{ borderTop:'1px solid rgba(255,255,255,0.1)', marginTop:8, paddingTop:12, display:'flex', justifyContent:'space-between' }}>
            ...
            <span style={{ fontSize:16, fontWeight:700, color:'#C9A84C' }}>£70,000 inc VAT</span>
          </div>
        </div>
      ...
      <div style={{ background:'#0D1117', border:'1px solid #21262D', borderRadius:10, padding:'16px 20px' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
          <div style={{ fontSize:13, fontWeight:700, color:'var(--cream)', letterSpacing:'0.06em' }}>£70,000 INC VAT TOTAL · 100% DEPLOYED DAY 1</div>
          <div style={{ fontSize:12, color:'#9CA3AF' }}>VAT on startup costs (~£5,833) reclaimed in Q1 — credited against first HMRC VAT return (August 2026)</div>
        </div>
