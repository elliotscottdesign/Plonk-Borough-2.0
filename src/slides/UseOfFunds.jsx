...
export default function UseOfFunds() {
  const funds = [
    { label:'Hardware from Liquidators', color:'#4FC3F7', pct:16, amount:24000, vat:'+VAT', icon:'ð§', desc:'Physical bar & kitchen equipment â purchased at liquidation pricing. Operational on Day 1.' },
    { label:'Plonk IP & Goodwill', color:'#C9A84C', pct:48, amount:72000, vat:'+VAT', icon:'â­', desc:'Brand, gaming IP, customer data and trading goodwill â the proven revenue-generating asset.' },
    { label:'Stock & Supplier Restart', color:'#2DD4BF', pct:8, amount:12000, vat:'+VAT', icon:'ð¦', desc:'Opening stock, supplier agreements and software subscriptions to trade from Day 1.' },
    { label:'Rent Deposit (3 months)', color:'#8B5CF6', pct:18, amount:27078, vat:'inc VAT', icon:'ð ', desc:'Security deposit held by landlord covering May, Jun, Jul 2026.' },
    { label:'Working Capital Buffer', color:'#6B7280', pct:10, amount:14922, vat:null, icon:'ð¼', desc:'Staged into business per cash flow model. Covers early trading before revenue covers costs.' },
  ]
  ...
  return (
    <div style={{ maxWidth:1100, margin:'0 auto', padding:'0 4px' }}>
      <div style={{ marginBottom:24 }}>
        ...
        <h1 style={{ fontSize:'clamp(1.8rem,3.5vw,2.8rem)', fontWeight:900, color:'var(--cream)', marginBottom:8, textTransform:'uppercase' }}>Where Your Â£150,000 Goes</h1>
        <p style={{ fontSize:14, color:'#9CA3AF' }}>Every pound deployed on Day 1 of reopening â no funds held in reserve outside the business</p>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginBottom:20 }}>
        <div style={{ background:'#0D1117', border:'1px solid #21262D', borderRadius:10, padding:24 }}>
          <div style={{ fontSize:12, color:'#4FC3F7', letterSpacing:'0.12em', textTransform:'uppercase', fontWeight:600, marginBottom:20 }}>Fund Allocation â Visual Breakdown</div>
          ...
          <div style={{ borderTop:'1px solid rgba(255,255,255,0.1)', marginTop:8, paddingTop:12, display:'flex', justifyContent:'space-between' }}>
            ...
            <span style={{ fontSize:16, fontWeight:700, color:'#C9A84C' }}>Â£150,000</span>
          </div>
        </div>
      ...
      <div style={{ background:'#0D1117', border:'1px solid #21262D', borderRadius:10, padding:'16px 20px' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
          <div style={{ fontSize:13, fontWeight:700, color:'var(--cream)', letterSpacing:'0.06em' }}>Â£150,000 TOTAL Â· 100% DEPLOYED DAY 1</div>
          <div style={{ fontSize:12, color:'#9CA3AF' }}>VAT on startup costs (Â£18,000) reclaimed in Q1 â credited against first HMRC VAT return (August 2026)</div>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12 }}>
          <div style={{ background:'rgba(234,88,12,0.1)', border:'1px solid rgba(234,88,12,0.3)', borderRadius:8, padding:'14px 18px', textAlign:'center' }}>
            ...
            <div style={{ fontSize:24, fontWeight:800, color:'#EA580C', marginBottom:4 }}>Â£135,078</div>
          ...
          <div style={{ background:'rgba(201,168,76,0.08)', border:'2px solid rgba(201,168,76,0.4)', borderRadius:8, padding:'14px 18px', textAlign:'center' }}>
            ...
            <div style={{ fontSize:24, fontWeight:800, color:'#C9A84C', marginBottom:4 }}>Â£14,922</div>
          ...
          <div style={{ background:'rgba(45,212,191,0.08)', border:'1px solid rgba(45,212,191,0.3)', borderRadius:8, padding:'14px 18px', textAlign:'center' }}>
            ...
            <div style={{ fontSize:24, fontWeight:800, color:'#2DD4BF', marginBottom:4 }}>Â£18,000</div>
            <div style={{ fontSize:12, color:'#9CA3AF' }}>Recovered Q1 â August 2026</div>
          </div>
        </div>
      </div>
    </div>
