    ...
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ marginBottom: 28 }}>
        ...
        <h2 className="serif" style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', lineHeight: 1.05, color: 'var(--cream)', marginBottom: 14 }}>
          {fmt(HACKNEY_DEAL.investment)} for {pct(HACKNEY_DEAL.investorEq, 0)} of the post-money business.
        </h2>
        <p style={{ fontSize: 17, color: 'var(--cream-dim)', maxWidth: 720, lineHeight: 1.6 }}>
          Priced at <strong style={{ color: 'var(--cream)' }}>{HACKNEY_DEAL.multiple.toFixed(2)}× verified 2025 EBITDA</strong> — below the 4.1× hospitality sector average (Houlihan Lokey H1 2025). The 50/50 split compresses the entry multiple in the investor's favour while exit is held at the sector benchmark.
        </p>
      </div>
      ...
      <div className="card" style={{ padding: 24, marginBottom: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0, alignItems: 'stretch' }}>
          {[
            ...
            { label: '× Multiple',      value: `${HACKNEY_DEAL.multiple.toFixed(2)}×`, sub: 'Implied by 50/50 split' },
          ...
          <div className="card-highlight" style={{ padding: 22 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                <span style={{ color: 'var(--cream-dim)' }}>Founder</span>
                <span style={{ color: 'var(--cream)', fontVariantNumeric: 'tabular-nums' }}>{pct(HACKNEY_DEAL.founderEq, 0)}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                <span style={{ color: 'var(--cream-dim)' }}>Investor</span>
                <span style={{ color: 'var(--gold)', fontVariantNumeric: 'tabular-nums' }}>{pct(HACKNEY_DEAL.investorEq, 0)}</span>
              </div>
              <div style={{ fontSize: 11, color: 'var(--cream-dim)', marginTop: 6 }}>
                Single share class — no A/B distinction.
              </div>
            </div>
          </div>

          <div className="card" style={{ padding: 22 }}>
            <div style={{ fontSize: 10, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: 10 }}>50/50 pro-rata</div>
            <div style={{ fontSize: 13, color: 'var(--cream-dim)', lineHeight: 1.6 }}>
              No founder priority slice. After the 8% preferred return and the reserve gate, every pound of profit splits 50/50 with the founder. Pure pass-through.
            </div>
          </div>
        </div>
      </div>
      ...
      {/* SECTOR BENCHMARK */}
      <div className="card" style={{ padding: 22 }}>
        <div style={{ fontSize: 10, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: 14 }}>Multiple in context</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10 }}>
          {[
            { label: 'UK mid-market',        value: `${HACKNEY_SECTOR.ukMidMarketAvg.toFixed(1)}×`, tone: 'dim' },
            ...
            { label: 'Entry — this deal',     value: `${HACKNEY_DEAL.multiple.toFixed(2)}×`, tone: 'gold' },
            { label: 'Exit assumption (Y5)',  value: `${HACKNEY_DEAL.exitMultiple.toFixed(1)}×`, tone: 'dim' },
          ].map(c => (
            <div key={c.label} style={{ padding: '12px 14px', background: c.tone === 'gold' ? 'rgba(201,168,76,0.1)' : 'var(--ink-3)', borderRadius: 8, border: c.tone === 'gold' ? '1px solid rgba(201,168,76,0.4)' : '1px solid rgba(201,168,76,0.08)' }}>
              ...
              <div className="serif" style={{ fontSize: '1.4rem', color: c.tone === 'gold' ? 'var(--gold)' : 'var(--cream)', lineHeight: 1 }}>{c.value}</div>
            </div>
          ))}
        </div>
        <div style={{ fontSize: 11, color: 'var(--cream-dim)', marginTop: 12, lineHeight: 1.5 }}>
          Source: CLFI M&amp;A Monitor H1 2025 · Houlihan Lokey hospitality sector data. Distressed/liquidation comparable trades 2–3×. The 50/50 split compresses the entry multiple below sector average; exit returns to the benchmark.
        </div>
      </div>
    </div>
  )
