...
import Interviewees from './Interviewees.jsx'
...
export default function StaffRota() {
  ...
  const [trials, setTrials] = useState([])               // 🎓 interviewees ledger
  const [trialShifts, setTrialShifts] = useState([])     // their scheduled trial shifts
  ...
  const load = async ({ silent = false } = {}) => {
    ...
    try {
      const r = await rotaLoad(); setStaff(r.staff || []); setShifts(r.shifts || []); setClaims(r.claims || []); setNotes(r.notes || []); setClocks(r.clocks || []); setAvailability(r.availability || []); setRotaRules(r.rotaRules || null); setTrials(r.trials || []); setTrialShifts(r.trialShifts || [])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', position: 'sticky', top: 0, zIndex: 20, background: 'var(--ink)', paddingTop: 6, paddingBottom: 6 }}>
        {[['team', '👥 Team'], ['rota', '🗓️ Rota'], ['availability', '📅 Availability'], ['ai', '🤖 Ai Builder'], ['pay', '💷 Pay'], ['interviews', '🎓 Interviews'], ['checklists', '📋 Checklists'], ['training', '🎓 Training'], ['menus', '🍽️ Menus'], ['settings', '⚙️ Settings']].map(([k, lbl]) => (
      ...
      {view === 'rota' ? (
        <RotaCalendar staff={staff} shifts={shifts} claims={claims} notes={notes} clocks={clocks} availability={availability} rules={rotaRules} trialShifts={trialShifts} trials={trials} reload={load} />
      ) : view === 'availability' ? (
        ...
        <Interviewees trials={trials} trialShifts={trialShifts} reload={load} />
      ) : view === 'checklists' ? (
        <ChecklistsHub />
      ) : view === 'training' ? (
