...
- **Xero: every call goes through [infra/xero_client.py](infra/xero_client.py).** Never write a one-off script that talks to `api.xero.com` directly. Xero allows 5,000 calls a day and locks the connection for fifteen hours when that runs out — which happened on 15 Sep 2026 because a matching job re-downloaded the same 290 documents repeatedly, and cost the founder an afternoon he had said was a deadline. The client caps the day at 3,000, caches file content forever (files in Xero are immutable, so a second download is always a bug), caches lists for 30 minutes, and throttles to 50 calls a minute. When the budget is spent, stop and do the remaining work offline against the cache — all the matching is local computation anyway.
...
- **Dates are TYPED, never scroller-pickers (house rule, Aug 2026).** Anywhere the team webapp asks for a date, use the shared [src/lib/DateField.jsx](src/lib/DateField.jsx) (typed DD/MM/YYYY with auto-slashes, speaks ISO, safe against wiping a saved value) — never `<input type="date">`, whose mobile wheel-picker the founder has banned (worst case: new staff scrolling back decades to their date of birth). Pass `yearMin`/`yearMax` to fit the field (DOB = `1930` / `now−14`; defaults are ±5 years). The only exception is the Hackney investor deck's legal sign block, which predates the rule.
...
| `admin` | `docs/admin/**` — the account register: which third-party services the business runs on, which Google account owns each, and what it would cost to lose them. **Owns no app code**, so it never collides with another lane. | *(none)* |
