...
row), plus 3 'made' prep products (Fresh lime juice, Fresh lemon juice, Sugar syrup 1:1 —
kind='prep', source='made') with `bar_prep_recipes` batch definitions. Idempotent — re-running
refreshes prices and replaces lines. Generated by `scripts/recipesSeed.py` from the costing
sheet's machine-readable recipes; names aligned to the live till so GP joins with no mapping.
Bar lane: your tables, our data — shout if any of it collides with a seeding plan of yours.
`till` fn v5 deployed same day (new `hq` action: Z-read history + voucher digest).
