...
async function leagueSeedOrder(sb: any, run: any, active: any[]): Promise<string[]> {
  const { data: t } = await sb.from("tournaments").select("tournament_type").eq("id", run.tournament_id).maybeSingle();
  const discipline = (run.discipline_override || t?.tournament_type) === "doubles" ? "doubles" : "singles";
